...
6. Implement the webpage.
   ...
   - Paired figures in the same row must use `align-items: stretch` (not `start`) with inner flexbox + `object-fit: contain` to prevent height misalignment when aspect ratios differ.
